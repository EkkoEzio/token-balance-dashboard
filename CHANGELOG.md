...
### Fixed
- **测试弹真通知事故**：部分调度测试经 `refresh_now()` 真实调用 macOS 通知（未替身拦截），用空数据触发「余额仅剩 ¥0.00」弹到用户屏幕（含从未配置 key 的 SiliconFlow）。新增 `tests/conftest.py` 全局隔离：所有测试自动禁用真实通知、数据目录重定向到临时目录。
- **余额告警对缺字段数据不误报**：`_alert_deepseek` 在数据缺 `total_balance` 时直接跳过，不再把异常数据当 ¥0 触发告警。
- **告警通知周期感知去重**：窗口型告警（5 小时/7 天/月度）改为「**同一周期内最多通知一次**」，周期内阈值抖动、短暂恢复又耗尽、fetch 失败恢复均不重弹；窗口重置（重置时间变化）后再次耗尽才算新事件、正常再通知。此前按「告警消失即重置」，阈值附近抖动会反复弹通知。
- **通知去重状态持久化**（`data/notified.json`）：此前状态纯内存，重启即失忆——启动时前端过期自检触发单卡刷新会在补种完成前把存量告警整体重发。现在重启恢复状态，不再重弹。
- 回归保障：隐藏（禁用）的 provider 不产生任何告警通知（原有行为，加测试锁死）。

---

## [v1.1] — 2026-08-15

接入 2 家新服务(看板升至 7 家)、修复设置页开关死锁、完成开源发布。
